...
int main(int argc, char **argv)
{
  ...
  Real k = 75.;   // wave number
  Real theta = 0; // incidence angle

  // mesh
  Real r1 = 1., r2 = 0.1, alpha = 2; // alpha > 1
  ...
  // =================================
    ...
    BilinearForm mlffBEM= intg(gamma, p*q);
    BilinearForm blffBEM= 0.5*intg(gamma, p*q)-intg(gamma, gamma, p*ndotgrad_y(G)*q,IMie);

    TermMatrix KfBEM(blffBEM,"KfBEM"), MfBEM(mlffBEM,"MfBEM");
    TermVector P = directSolve(KfBEM,MfBEM*Uinc);
    ...
    cpuTime("cpu time for full BEM");
    elapsedTime("elapsed time for full BEM");

    TermVector Ud_full_BEM = integralRepresentation(ur, omega, intg(gamma, ndotgrad_y(G)*P, IMir));

    saveToFile("Ud_full_BEM", Ud_full_BEM, vtu); // scattered field in exterior domain
    TermVector Ut_full_BEM = Ud_full_BEM + TermVector(ur,omega,fuinc);
    saveToFile("Ut_full_BEM", Ut_full_BEM, vtu); // Total field in exterior domain
  ...
  // =================================
   ...
   /*
    ...
    cpuTime("cpu time to initialize BEM BEM");
    elapsedTime("elapsed time to initialize BEM BEM");
    ...
    printf("\n \n  temps BEM BEM : \n \n");
    cpuTime("cpu time for BEM BEM");
    elapsedTime("elapsed time for BEM BEM");
    ...
    TermVector Ud_BEM_BEM = (Uext_1 + Uext_2);

    saveToFile("Ud_BEM_BEM", Ud_BEM_BEM, vtu); // scattered field in exterior domain

    TermVector Ut_BEM_BEM = Ud_BEM_BEM + TermVector(ur,omega,fuinc);

    saveToFile("Ut_BEM_BEM", Ut_BEM_BEM, vtu); // scattered field in exterior domain
    
    TermVector err = Ud_BEM_BEM-Ud_full_BEM;

    std::cout<< "\n  BEM BEM : \n" << norm2(err)/norm2(Ud_full_BEM);
  ...
  // =================================
    ...
    cpuTime("start Kirchoff BEM");
    elapsedTime("start Kirchoff BEM");
    for (int i=1;i<=n;i++){
      ...
      if(i==1){
        U_2 += 2*integralRepresentation(p_2, gamma2, intg(gamma1, ndotgrad_y(G)*U_1, IMir2));
      }
      else{ U_2 = 2*integralRepresentation(p_2, gamma2, intg(gamma1, ndotgrad_y(G)*U_1, IMir2));}

      TermVector Pn = factSolve(K_f,M*U_2);

      P2+= Pn;

      U_1= integralRepresentation(p_1, gamma1, intg(gamma2, ndotgrad_y(G)*Pn, IMir2));
      grad_U_1= integralRepresentation(g_1,gamma1,intg(gamma2,grad_x(ndotgrad_y(G))*p_2,IMir2),Pn);


    }

    printf("\n \n  temps Kirchoff BEM : \n \n");
    cpuTime("fin Kirchoff BEM");
    elapsedTime("fin Kirchoff BEM");
...
/*
  ...
  Unknown p_1(H_gamma1, _name = "p_1");
  TestFunction q_1(p_1, _name = "q_1");
  Unknown p_2(H_gamma2, _name = "p_2");
  TestFunction q_2(p_2, _name = "q_2");
  ...
  vector<Vector<real_t>> ns;

  cpuTime("cpu time to initialize UTD BEM");
  elapsedTime("elapsed time to initialize UTD BEM");
  ...
  for (int i = 1; i <= n; i++)
  {
    ...
    interpolatedNormals(H_gamma1, ns);

    UTD(U_1, grad_U_1, ns, k, r1, F);
  ...
  cpuTime("cpu time for UTD BEM");
  elapsedTime("elapsed time for UTD BEM");
  ...
  saveToFile("Ud_UTD_BEM", Ud, vtu); // scattered field in exterior domain

  TermVector Ut = Ud + TermVector(ur, omega, fuinc);

  saveToFile("Ut_UTD_BEM", Ut, vtu); // scattered field in exterior domain
